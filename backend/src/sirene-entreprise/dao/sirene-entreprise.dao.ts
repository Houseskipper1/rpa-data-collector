...
export class SireneEntrepriseDao {
  ...
  async findAll(): Promise<SireneEntreprise[]> {
    return await this._sireneEntrepriseModel
      .find({ name: { $ne: '' } })
      .sort({ name: 1 })
      .limit(1000)
      .exec();
  ...
  async findAllLimitless(sorted: boolean) {
    return this._sireneEntrepriseModel
      .find({ name: { $ne: '' } })
      .sort({ _id: sorted ? 1 : -1 })
      .cursor();
  }
  ...
  async save(
    sireneEntrepriseEntity: SireneEntrepriseEntity,
  ): Promise<SireneEntreprise> {
    const SireneEntreprise = new this._sireneEntrepriseModel(
      sireneEntrepriseEntity,
    );
